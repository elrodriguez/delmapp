            ...
            <form class="needs-validation <?php echo e($errors->any() ? 'was-validated' : ''); ?>" novalidate="">
                <div class="form-row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label" for="category_id_new"><?php echo app('translator')->get('restaurant::labels.categories'); ?>
                        </label>
                        <div wire:ignore>
                            <input type="text" id="justAnotherInputBox" placeholder="Escriba para filtrar"
                                autocomplete="off" />
                        </div>
                        <?php $__errorArgs = ['category_id_new'];
...
?>;
            let CategoryIds = <?php
    if (is_object($category_id_old) || is_array($category_id_old)) {
        echo "JSON.parse(atob('".base64_encode(json_encode($category_id_old))."'))";
    } elseif (is_string($category_id_old)) {
        echo "'".str_replace("'", "\'", $category_id_old)."'";
    } else {
        echo json_encode($category_id_old);
    }
?>;
        ...
        function saveRestCommand() {
            ...
            window.livewire.find('<?php echo e($_instance->id); ?>').set('category_id_new', cat);
            window.livewire.find('<?php echo e($_instance->id); ?>').saveCommand();
        }
    </script>
</div>
<?php /**PATH C:\laragon\www\delmapp\Modules/Restaurant\Resources/views/livewire/commands/commands-edit.blade.php ENDPATH**/ ?>