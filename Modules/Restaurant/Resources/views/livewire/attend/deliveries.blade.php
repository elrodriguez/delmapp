    ...
    <div class="card mb-g rounded-top">
        <div class="card-header">
            <div class="input-group bg-white shadow-inset-2">
                ...
                <input wire:keydown.enter="brandSearch" wire:model.defer="search" type="text"
                    class="form-control border-left-0 bg-transparent pl-0" placeholder="Escriba aquí...">
                <div class="input-group-append">
                    <button wire:click="brandSearch" class="btn btn-default waves-effect waves-themed"
                        type="button">{{ __('labels.search') }}</button>
                    @can('restaurante_panel_delivery_nuevo')
                        <a href="{{ route('restaurant_panels_deliveries_create') }}"
                    ...
                    @if(count($deliveries)>0)
                        @foreach ($deliveries as $key => $delivery)
                            <tr>
                                ...
                                <td class="text-center tdw-50 align-middle">
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu"
                                            ...
                                            @can('restaurante_administracion_marcas_editar')
                                                <a href="{{ route('restaurant_brands_edit', $delivery->id) }}"
                                                    class="dropdown-item">
                                            ...
                                            @can('restaurante_administracion_marcas_eliminar')
                                                <button onclick="confirmDelete({{ $delivery->id }})" type="button"
                                ...
                                <td class="align-middle">{{ $delivery->description }}</td>
        ...
        <div class="card-footer  pb-0 d-flex flex-row align-items-center">
            <div class="ml-auto">{{ $deliveries->links() }}</div>
        </div>
    </div>
