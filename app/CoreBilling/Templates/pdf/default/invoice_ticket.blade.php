...
<body>
    ...
    <table class="full-width">
        ...
        <tr>
            <td class="text-center pt-3 border-top">
                <h4>{{ $documentType->description }}</h4>
            </td>
        </tr>
        <tr>
    ...
    <table class="full-width">
            ...
            @foreach ($invoice as $item)
                @if ($item->date_of_due != $document->date_of_issue)
                    <tr>
                        ...
                        <td>
                            <p class="desc">{{ $item->date_of_due }}</p>
                        </td>
                    </tr>
                @endif
            @endforeach
        ...
        <tr>
            ...
            <td>
                <p class="desc">{{ $customer->full_name }}</p>
            </td>
        </tr>
        <tr>
            <td>
                <p class="desc">{{ $identity_document_type->description }}:</p>
            </td>
            <td>
        ...
        @if ($document->detraction)
            ...
            <tr>
                ...
                <td>
                    <p class="desc">{{ $currency_type->symbol }} {{ $document->detraction->amount }}</p>
                </td>
            </tr>
        ...
        @isset($document->quotation->delivery_date)
            <tr>
                ...
                <td>
                    <p class="desc">{{ $document->quotation->delivery_date->format('Y-m-d') }}</p>
                </td>
            </tr>
        @endisset
...
@endif --}}
    ...
    <table class="full-width mt-10 mb-10">
        ...
        <tbody>
            ...
            @if ($document->total_exportation > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">OP. EXPORTACIÓN: {{ $currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_free > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">OP. GRATUITAS: {{ $currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_unaffected > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">OP. INAFECTAS: {{ $currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_exonerated > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">OP. EXONERADAS: {{ $currency_type->symbol }}
                    </td>
            ...
            @if ($document->total_discount > 0)
                <tr>
                    <td colspan="4" class="text-right font-bold desc">DESCUENTO TOTAL: {{ $currency_type->symbol }}
                    </td>
    ...
    <table class="full-width">
        ...
        <tr>
            @if ($legend->code == '1000')
                <td class="desc pt-3">Son: <span class="font-bold">{{ $legend->value }}
                        {{ $currency_type->description }}</span></td>
    ...
    @else
        ...
        @if ($payments->count())
            ...
            @foreach ($payments as $row)
                <tr>
                    <td class="desc">&#8226; {{ $row->description }} -
                        {{ $row->reference ? $row->reference . ' - ' : '' }} {{ $currency_type->symbol }}
                        {{ number_format($row->payment + $row->change, 2, '.', '') }}</td>
                </tr>
            @endforeach
        @endif
        <tr>
        ...
        <tr>
            <td class="text-center desc pt-5">Para consultar el comprobante ingresar a {{ route('micomprobante') }}
            </td>
        </tr>
    </table>
