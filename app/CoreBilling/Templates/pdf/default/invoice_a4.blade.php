...
@endisset
            ...
            @if ($document->total_plastic_bag_taxes > 0)
                <tr>
                    <td colspan="{{ $colspan }}" class="text-right font-bold">ICBPER:
                        {{ $currency_type->symbol }}</td>
                    <td class="text-right font-bold">{{ number_format($document->total_plastic_bag_taxes, 2) }}</td>
                </tr>
            @endif
            <tr>
                <td colspan="{{ $colspan }}" class="text-right font-bold">IGV: {{ $currency_type->symbol }}
                </td>
            ...
            @if ($document->perception)
                <tr>
                    <td colspan="{{ $colspan }}" class="text-right font-bold"> IMPORTE TOTAL:
                        {{ $currency_type->symbol }}</td>
                    <td class="text-right font-bold">{{ number_format($document->total, 2) }}</td>
                </tr>
                <tr>
                    <td colspan="{{ $colspan }}" class="text-right font-bold">PERCEPCIÓN:
                        {{ $currency_type->symbol }}</td>
            ...
            @if ($balance < 0)
                <tr>
                    <td colspan="{{ $colspan }}" class="text-right font-bold">VUELTO:
                        {{ $currency_type->symbol }}</td>
    ...
    <table class="full-width">
        <tr>
            <td width="65%" style="text-align: top; vertical-align: top;">
                ...
                @if ($legend->code == '1000')
                    <p>Son: <span class="font-bold">{{ $legend->value }}
                            {{ $currency_type->description }}</span></p>
    ...
    @if ($payments->count())
        ...
        <table class="full-width">
            ...
            @foreach ($payments as $row)
                <tr>
                    <td>&#8226; {{ $row->description }} - {{ $row->reference ? $row->reference . ' - ' : '' }}
